    ...
    if user_input.lower() == 'y':
        try:
            ...
            for phone in phoneNumberL:
                try:
                    cursor.execute("INSERT INTO phone_numbers (phone) VALUES (%s);", (phone[0],))
                except Exception as e:
                    update.message.reply_text(f'Ошибка при сохранении номера {phone[0]}: {e}')
                    continue
        ...
        try:
            ...
            for email in EmailL:
                try:
                    ...
                    update.message.reply_text(f'Ошибка при сохранении email {email}: {e}')
                    continue
...
def get_repl_logs(update: Update, context):
    try:
        ...
        update.message.reply_text(client_connect_and_execute('pwd'))
    return ConversationHandler.END
